                            ...
                            plugins: {
                                title: {
                                    ...
                                    text: (ctx3) => {
                                        ...
                                        } = ctx3.chart.options.interaction;
    ...
    <?php if ($attribute == 2) { ?>
        ...
        <div class="w3-round mt-5" style='background-color:#272B34;color:gray;'>
            <div class="row w-100">
                ...
                $value = '';
                // $PrevVal = '';
                $s = "SELECT DATE(fd_uploaded_on) AS `upload_date`, COUNT(*) AS `this_month_total` FROM `tb_history` WHERE `fd_admin_id` = $emid 
                    AND `fd_uploaded_on` >  '$s_date' AND `fd_uploaded_on` < '$e_date' AND fd_status = 0 AND fd_delete = 0 GROUP BY DATE(fd_uploaded_on)";
                            ...
                            plugins: {
                                title: {
                                    ...
                                    text: (ctx_2) => {
                                        const {
                                            axis = 'xy', intersect, mode
                                        } = ctx_2.chart.options.interaction;
                                        return 'Mode: ' + mode + ', axis: ' + axis + ', intersect: ' + intersect;
                                    }
                                },
                            }
                    ...
                    var myChart3 = new Chart(ctx3, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                ...
                                backgroundColor: floatArray.map(function(value) {
                                    return value === max3 ? '#39FF14' : value === min3 ? '#f70d1a' : 'rgb(0, 255, 255)';
                                }),

                                pointBackgroundColor: floatArray.map(function(value) {
                                    return value === max3 ? '#39FF14' : value === min3 ? '#f70d1a' : 'rgb(0, 255, 255)';
                                }),

                                pointBorderColor: floatArray.map(function(value) {
                                    return value === max3 ? '#39FF14' : value === min3 ? '#f70d1a' : 'rgb(0, 255, 255)';
                                }),

                                pointRadius: floatArray.map(function(value) {
                                    return value === max3 ? '5' : value === min3 ? '5' : '0';
                                }),

                                pointStyle: floatArray.map(function(value) {
                                    return value === max3 ? 'rectRot' : value === min3 ? 'rectRot' : 'circle';
                                }),
